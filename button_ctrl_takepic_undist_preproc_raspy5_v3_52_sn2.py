...
# -*- coding: utf-8 -*-
#
#  button_ctrl_takepic_undist_preproc_raspy5_v3_52_sn2.py  RELEASE intermittent error fix in left page crop  
# 11 09 2026 
# Algorythm Talking Scanner ??
...
# 
# Command line: python button_ctrl_takepic_undist_preproc_raspy5_v3_52_sn2.py 
...
# function returns the row number of the upper page border
def find_upr_pg_brd(bin_img, thrd):
	white_per_row = np.count_nonzero(bin_img == 255, axis=1)
	for row_num, white_count in enumerate(white_per_row):
		if white_count > thrd:
			return row_num
	return 0


# function returns the column number of the left page left border
def find_left_page_left_brd(bin_img, thrd):
	white_per_clmn = np.count_nonzero(bin_img == 255, axis=0)
	for clmn_num, white_count in enumerate(white_per_clmn):
		if white_count > thrd:
			return clmn_num
 

# function returns the column number of the right page right border
def find_right_page_right_brd(bin_img, thrd):
	h, w = bin_img.shape[:2]
	white_per_clmn = np.count_nonzero(bin_img == 255, axis=0)
	for i in range(w - 1, 0, -1):
		if white_per_clmn[i] > thrd:
			return i
	return 0
...
# Define the right camera imx219 sn2 calibration matrix (to be imported from the calibration script results)
camera_matrix_R = np.array([[9.09996224e+03,   0.00000000e+00,  1.69047669e+03],
     [  0.00000000e+00,  9.12347786e+03,  1.33655993e+03],
     [  0.00000000e+00,   0.00000000e+00,   1.00000000e+00]])
# Define the right camera distortion coefficients (to be imported from the calibration script results)
dist_coefs_R = np. array([-2.15868680e+00, 2.42681757e+00, 7.54623999e-03,  1.45814853e-02, 6.04188721e+01])
...
def main():
	...
	gray_img_L = cv.cvtColor(image_L, cv.COLOR_BGR2GRAY)		# CHANGED TO GRAY CONVERSION
	clahe = cv.createCLAHE(clipLimit=1.5, tileGridSize=(8,8))	# CHANGED CLIPLIMIT FROM 2.0 TO 1.5
	cl_equalized_gray_L = clahe.apply(gray_img_L)
	thres_val, img_L = cv.threshold(cl_equalized_gray_L, 125, 255, cv.THRESH_BINARY)	# THRESHOLD 150 AND BINARY CONVERSION
	hL,  wL = img_L.shape[:2]
	newcameramtx_L, roi_L = cv.getOptimalNewCameraMatrix(camera_matrix_L, dist_coefs_L, (wL,hL), 1, (wL,hL))
				
	# undistort image R
	dst = cv.undistort(img, camera_matrix_R, dist_coefs_R, None, newcameramtx)
	
	# undistort image L
	dst_L = cv.undistort(img_L, camera_matrix_L, dist_coefs_L, None, newcameramtx_L)
	...
	# crop the image L 
	xL, yL, wL, hL = roi_L
	dst_L = dst_L[yL:yL+hL, xL:xL+wL]
	
	# rotate the image R 90 counterclockwise	
	dstRotate90 = cv.rotate(dst, cv.ROTATE_90_COUNTERCLOCKWISE)

	# rotate the image L 90 clockwise
	dstRotate90_L = cv.rotate(dst_L, cv.ROTATE_90_CLOCKWISE)
	...
	hL2, wL2 = dstRotate90_L.shape[:2] 
	L_img_up_brd_line = find_upr_pg_brd(dstRotate90_L, thres_wht_pxs) + 30
	L_img_crop_left_line = find_left_page_left_brd(dstRotate90_L, thres_wht_pxs) + 60

	# Define names to save scanned image R file 
	...
	# Define names to save scanned image L file 
	NextImgFileName_L = "/tmp/TalkingScanner/page_1.png"
	# Save scanned image L to file 
	cv.imwrite(NextImgFileName_L, dstRotate90_L[L_img_up_brd_line:, L_img_crop_left_line:wL2 - L_img_crop_right_line])
	print('Scanned image L saved to file name : ', NextImgFileName_L)
